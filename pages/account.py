...
# AUTH QUERIES
# ══════════════════════════════════════════════════════════════════════════════
def _register_user(conn, username, email, password,
                   first_name=None, last_name=None,
                   school=None, grade_class=None):
        ...
        cur.execute(
            """INSERT INTO users
               (username, email, password_hash, salt,
                first_name, last_name, school, grade_class)
...
def _login_user(conn, identifier, password):
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute(
...
# STATS QUERIES
# ══════════════════════════════════════════════════════════════════════════════
def _get_overall_stats(conn, user_id):
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("""
            SELECT
...
def _get_last_sessions(conn, user_id, limit=5):
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("""
...
def _accuracy_bar(accuracy):
    acc = float(accuracy or 0)
    color = "#4ade80" if acc >= 70 else ("#fbbf24" if acc >= 40 else "#f87171")
    return (f'<div style="background:#1e293b;border-radius:6px;height:8px;'
            f'width:100%;margin-top:4px;">'
            f'<div style="background:{color};border-radius:6px;height:8px;'
            f'width:{min(acc,100):.0f}%;"></div></div>')


def _logout():
    for key in ['user_id', 'username', 'user_email',
                'user_first_name', 'user_last_name',
                'user_school', 'user_grade_class']:
    ...
    with tab_login:
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            password = st.text_input(
                "Пароль", type="password", placeholder="Введіть пароль"
            )
        ...
        if submitted:
            if not identifier or not password:
                ...
                conn = get_connection()
                if conn:
                    ok, result = _login_user(conn, identifier, password)
                    conn.close()
                    if ok:
    ...
    with tab_reg:
        ...
        with st.form("register_form", clear_on_submit=True):
            ...
            new_username = st.text_input(
                "Ім'я користувача (логін)", placeholder="мінімум 3 символи"
            )
            new_email = st.text_input("Email", placeholder="your@email.com")
            col_p1, col_p2 = st.columns(2)
            with col_p1:
                new_pass = st.text_input(
                    "Пароль", type="password", placeholder="мінімум 6 символів"
                )
            with col_p2:
                new_pass2 = st.text_input(
                    "Повторіть пароль", type="password", placeholder="повторіть пароль"
                )

            reg_submitted = st.form_submit_button(
                "Зареєструватись", use_container_width=True
            )
...
def show_dashboard():
    ...
    # Meta pills
    meta_parts = []
    if school:      meta_parts.append(f"🏫 {school}")
    if grade_class: meta_parts.append(f"📋 {grade_class} клас")
    meta_html = "&nbsp;&nbsp;·&nbsp;&nbsp;".join(
    ...
    with col_title:
        st.markdown(f"""
        <h2 style="color:#f1f5f9;margin-bottom:2px;">👤 {full_name}</h2>
        <p style="color:#64748b;margin-top:0;
                  margin-bottom:{'8px' if meta_html else '0'};">@{username}</p>
    ...
    overall  = _get_overall_stats(conn, user_id)
    sessions = _get_last_sessions(conn, user_id, limit=5)
    conn.close()

    if not overall:
        st.error("❌ Не вдалося завантажити статистику.")
        return
    ...
    with c6:
        ...
        <div style="background:#1e293b;border-radius:12px;padding:20px;">
          ...
          <div style="font-size:1.6rem;font-weight:800;
                      color:{_grade_color(avg_g)};">{avg_g:.1f} / 12</div>
        </div>""", unsafe_allow_html=True)
    with c7:
        ...
        <div style="background:#1e293b;border-radius:12px;padding:20px;">
          ...
          <div style="font-size:1.6rem;font-weight:800;
                      color:{_grade_color(best_g)};">{best_g} / 12</div>
    ...
    if not sessions:
        st.info("📭 Ви ще не завершили жодної сесії. "
                "Перейдіть до **Тренажера** та почніть!")
    else:
        st.markdown("""
        <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;
                    gap:8px;padding:10px 16px;background:#0f172a;
                    border-radius:8px 8px 0 0;color:#64748b;font-size:0.8rem;
        ...
        for i, s in enumerate(sessions):
            ...
            radius   = "border-radius:0 0 8px 8px;" if i == len(sessions) - 1 else ""

            st.markdown(f"""
            <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;
                        gap:8px;padding:12px 16px;background:{bg};
                        align-items:center;{radius}">
